<template>
  <div class="container" ref="containerRef">
    ...
    <div class="right-panel" :style="{ width: rightWidth + 'vw' }">
      <ImgShow :processTimeProp="processTimeProp" :rightWidthProp="rightWidth" />
    </div>
  </div>
</template>
...
// 左右面板宽度，单位是 vw
const leftWidth = ref(49.5);
const rightWidth = ref(49.5);
...
const onDrag = (e) => {
  ...
  if (containerRef.value) {
    ...
    // 计算新的右侧宽度
    const newRightWidth = 100 - newLeftWidth - 1; // 减去分割线 1vw
    ...
    if (newRightWidth < MIN_RIGHT_WIDTH_VW) {
      ...
      // 计算：允许的最小右侧宽度对应的左侧宽度
      const allowedLeftWidth = 100 - MIN_RIGHT_WIDTH_VW - 1;

      // 限制左侧，从而保证右侧 >= 30vw
      newLeftWidth = Math.min(newLeftWidth, allowedLeftWidth);

      // 重新计算右侧宽度
      const finalNewRightWidth = 100 - newLeftWidth - 1;
    ...
    const newRightWidth = 100 - newLeftWidth - 1;
